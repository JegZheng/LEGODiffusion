# Modified from dataset_tool.py to process latent space dataset
...
def main(
    ...
    The output dataset format can be either an folder of compressed .npy or an uncompressed zip archive.
    ...
    {
        "labels": [
            ["00000/img00000000.npy",6],
            ["00000/img00000001.npy",9],
            ... repeated for every image in the datase
            ["00049/img00049999.npy",1]
        ]
    }
    ...
    \b
    python latent_dataset_tool.py --source imagenet_path --dest compressed_imagenet_latent_path \\
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        with torch.no_grad():
            # transform image from array to torch.tensor and normalize to [-1,1]
            img = torch.from_numpy(img).permute(2, 0, 1).unsqueeze(0).to(device).to(torch.float32) / 127.5 - 1
        ...
        cur_image_attrs = {'width': img.shape[3], 'height': img.shape[2], 'channels': channels}
        ...
        # Save .npy files.
        save_name = os.path.join(archive_root_dir, archive_fname)
        byte_stream = io.BytesIO()
        np.save(byte_stream, img.squeeze(0).cpu().numpy()) # turn tensor to numpy array and save as bytes
        save_bytes(os.path.join(archive_root_dir, archive_fname), byte_stream.getvalue())
